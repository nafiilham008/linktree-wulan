...
<body class="bg-white text-gray-600 work-sans leading-normal text-base tracking-normal">
    ...
    <div class="carousel relative container mx-auto" style="max-width:1600px;">
        <div class="carousel-inner relative overflow-hidden w-full">
            ...
            @foreach ($slider as $index => $link)
                <!-- Slide -->
                <input class="carousel-open" type="radio" id="carousel-{{ $index + 1 }}" name="carousel"
                    aria-hidden="true" hidden="" @if ($index == 0) checked="checked" @endif>
                <div class="carousel-item absolute opacity-0" style="height:50vh;"
                    onclick="redirectToLink('{{ route('clicks.store', [$link->id, $link->title]) }}', '{{ $link->url }}')">
                    <div class="block h-full w-full mx-auto flex pt-6 md:pt-0 md:items-center bg-cover bg-right"
                        ...
                        <div class="container mx-auto">
                            <div
                                ...
                                <p class="text-black text-2xl my-4">{{ $link->title }}</p>
                                <a class="text-xl inline-block no-underline border-b border-gray-600 leading-relaxed hover:text-black hover:border-black"
                                    href="#"
                                    onclick="redirectToLink('{{ route('clicks.store', [$link->id, $link->title]) }}', '{{ $link->url }}')">Lihat
                                    Link</a>
                            </div>
                        </div>
                    </div>
                </div>
                <label for="carousel-{{ $index == 0 ? $links->count() : $index }}"
                ...
                <label for="carousel-{{ $index + 2 > $links->count() ? 1 : $index + 2 }}"
...
-->
        ...
        <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
            ...
            <div class="flex flex-wrap gap-4 justify-center">
                @foreach ($links as $link)
                    <div class="w-4/5 mx-auto sm:w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col justify-between border rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 cursor-pointer bg-white dark:bg-gray-800"
                        onclick="redirectToLink('{{ route('clicks.store', [$link->id, $link->title]) }}', '{{ $link->url }}')">

                        <!-- Display image if it exists -->
                        @if ($link->image)
                            <img class="mb-4 hover:grow rounded-t-lg" src="{{ asset('storage/' . $link->image) }}"
                                alt="{{ $link->title }}">
                        @endif
                        <div class="flex-1">
                            <div class="pt-3 flex items-center justify-between">
                                <p class="text-2xl font-semibold">{{ $link->title }}</p>
                            </div>
                        ...
                        <p class="mt-4 text-blue-400 hover:text-blue-700 transition-colors duration-300">
                            Link toko
                        </p>
    ...
    <script>
        function redirectToLink(storeRoute, url) {
            // Send a POST request
            fetch(storeRoute, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    // Once the POST request is complete, redirect to the link URL from the server response in a new tab
